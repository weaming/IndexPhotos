...
应用只索引原始文件，不复制或修改原始照片。结果页的删除决定必须经用户二次确认后执行，可选择移动到 macOS 废纸篓或永久删除；目录管理只移除应用索引，不删除磁盘上的实际目录。

“重复”和“相似”是两种不同的输出：只有字节完全相同的文件进入重复组；经过去污、磨皮、曝光调整、裁剪等处理的同源照片进入相似候选，供人工确认删除哪一张，不自动归入可删除重复组。
...
| `review_decisions` | 用户对相似候选的保留、标记左图待删除、标记右图待删除或跳过状态 |
...
轻微修改照片一律写入 `similarity_candidates`，关系类型标记为 `edited_same_photo` 或其他具体变换类型，不写入 `duplicate_groups`，也不自动删除。用户可在当前相似候选页选择保留两张、标记左图待删除、标记右图待删除或跳过；执行删除时，同目录同名主干的其他支持照片格式一并处理，非照片 sidecar 文件不处理。
...
- 左侧：可添加并记忆多个扫描目录、各目录会话状态和缓存状态；当前选中目录决定工具栏的开始、继续、暂停和取消目标，同一存储卷同时只运行一个目录扫描；
- 中间：扫描进度或重复组；
...
结果列表和缩略图网格使用稳定的候选 ID，采用 lazy 容器，缩略图从缓存异步加载。待人工处理只纳入 `score > 0.6` 的候选，已审核记录可单独查看；候选按 0.1 分数区间筛选，每页最多显示 24 条，分页提供首页和尾页；每条候选同一行显示两侧文件大小、关系、分数和算法来源（`exact`/`phash`/`vision`）。完全相同文件同时保留 `duplicate_groups`，并以 `exact-v1`、分数 1.0 的候选供统一人工处理。左侧目录栏提供多选开关，开启后通过目录行前的勾选框选择探测范围；选择多个根目录时只显示不同根目录之间的相似候选，单目录模式保留同目录候选。扫描主目录仍由当前高亮目录决定。默认使用全部算法来源；未手动选择分数区间时自动定位到当前范围内最高的非空区间。标记删除记录只在当前筛选存在时显示“移到废纸篓”和“永久删除”操作，执行成功后删除资产、候选、审核、特征和扫描记录，并重建派生结果索引。重新扫描只有在目录枚举完整后才清理确认消失的孤儿资产。SQLite 单独查询分桶计数和当前页，并为分数排序建立索引。切换目录、页或审核后保留旧页直到新数据就绪，避免白屏和整页抖动。所有耗时任务都在 actor 或后台任务中执行，不放在 View 的 `body`、初始化器或主线程。
