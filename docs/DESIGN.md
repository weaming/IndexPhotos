...
对小型常规照片，尽量在一次 HDD 读取中同时完成 BLAKE3、解码缩略图、pHash 和 embedding。大 RAW/TIFF 根据内存预算走分阶段路径；已完成的特征不会因为后续阶段中断而失效。
...
- 大文件读取使用分块流式 BLAKE3；缩略图使用 ImageIO 下采样，禁止把原始 20MP/50MP 图片直接交给 UI。
